...
| `sweep_eps_big.py` | Not a manuscript claim: measures `eps_q` for odd `q <= 29` over all 664,577 primes `p < 10^7` (SE 0.00061), censuses the constant-`chi(R)` fibres in the same pass, and records each fibre's density. Data in `ancillary/eps_q_10M.txt`; the negative result it establishes is written up in `../NOTES.md`. | ~45 min |
