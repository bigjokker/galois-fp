...
\[
\varphi'\equiv g^{m-1}\psi\pmod q,\qquad\psi=g B_r'-m B_r.
\]
The polynomial \(\psi\) has degree at most \(q+r-1\le 2q-2\) and depends on \(p\) only through \(r\) and \(m\bmod q\). Multiplicativity of the resultant together with Proposition~\ref{prop:noroots} yields \(\Res(g,f_p)=\prod_{a\in\F_q}f_p(a)=1\), so the factor \(g^{m-1}\) contributes \(1^{m-1}\) and
\begin{equation}
\label{eq:res}
\disc f_p\equiv(-1)^{(p-1)/2}\Res(\psi,\,g^m B_r+1)\pmod q.
\end{equation}
...
\[
\Pi(q)=\mathrm{lcm}\Bigl(4,\,q^2,\,\mathrm{lcm}_{1\le d\le 2q-2}(q^d-1)\Bigr).
\]
...
\begin{proof}
The right-hand side of~\eqref{eq:res} is defined for any odd integer \(n=mq+r>q\) not divisible by \(q\), via \(f_n=(x)_n+1\). Writing \(\mathrm{lc}(\psi)\) for the leading coefficient of \(\psi\),
\[
\Res(\psi,f_n)=\mathrm{lc}(\psi)^n\prod_{\psi(\beta)=0}f_n(\beta)^{m_\beta},
\]
the product over roots in an algebraic closure, with multiplicity. The sign \((-1)^{(n-1)/2}\) has period \(4\). The polynomial \(\psi\) itself is determined by \(r\) and \(m\bmod q\), hence by \(n\bmod q^2\); this is the only source of the factor \(q^2\) in \(\Pi(q)\). The power \(\mathrm{lc}(\psi)^n\) depends on \(n\bmod(q-1)\), and \(q-1\) divides the \(d=1\) term of the inner lcm.

Each factor \(f_n(\beta)=g(\beta)^m B_r(\beta)+1\) is an evaluation in a field. If \(\beta\in\F_q\) then \(g(\beta)=0\), so \(f_n(\beta)=1\). If \(\beta\notin\F_q\) then \(g(\beta)\ne0\), because every root of \(g\) lies in \(\F_q\), and \(g(\beta)^m\) has period \(\operatorname{ord}\bigl(g(\beta)\bigr)\) dividing \(q^d-1\) where \(d=[\F_q(\beta):\F_q]\le 2q-2\).
...
The bound is not always sharp: \(\Pi(3)=9360\) while the actual period is \(36\), and \(\Pi(5)\) is enormous while the period is \(600=\mathrm{lcm}(4,5^2,5^2-1)\). The inner lcm cannot be dropped in general, because \(\psi\) can have complementary factors of degree greater than \(1\). For \(q=3\) the six polynomials \(\psi\) can be read off by hand, and among odd \(p\) the residue \(\Res(\psi,f_p)\) is constant on each class modulo \(9\).
...
\begin{proof}
Write \(p=3m+r\) with \(r\in\{1,2\}\) and \(g=x^3-x\). The six values of \(\psi\) as \(r\in\{1,2\}\) and \(m\bmod 3\) are as follows. In each case \(\Res(\psi,f_p)\) is a constant \(R\in\{1,2\}\) on odd \(p\), recorded with the corresponding class \(p\bmod 9\).
...
The identification \(\Gal(f_p/\Q)=S_p\) for every odd prime \(p\) is now a covering question. A finite list of \(q\) would finish all large \(p\), but the data already refutes covering by \(q\le43\): the prime \(p=31511\equiv11\pmod{36}\) has least witness \(47\). The least witness appears unbounded, so all-\(p\) is unlikely to fall to a fixed finite set of congruences. What remains is to show that no prime survives in the (explicit, shrinking) bad classes at every level \(q<p\).
